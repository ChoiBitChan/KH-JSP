...
public class BoardDBBean {   
    ...
    public int getArticleCount(String searchValue, String searchText) throws Exception {
        Connection conn = null;
        ...
        try {
            ...
            String sql = "select count(*) from reply_board where " +searchValue+ " like '%"+searchText+"%'";
	...
	int x=-1;
        try {
            ...
            pstmt = conn.prepareStatement("select passwd from reply_board where num = ?");
	    ...
	    conn = getConnection();
            ...
            if(rs.next()){
            	...
            	if(dbpasswd.equals(passwd)){
            		pstmt = conn.prepareStatement("delete from reply_board where num=?");
                    pstmt.setInt(1, num);
                    pstmt.executeUpdate();
                    x= 1; //글삭제 성공
            	}else
    ...
    public List search(String searchValue, String searchText, int start, int end) throws Exception {
    	Connection conn = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        List articleList=null;
        ...
        try {
            ...
            pstmt = conn.prepareStatement(
            		//"select * from reply_board where " + searchValue + " like '%" + searchText + "%'"
            		"select num,writer,email,subject,passwd,reg_date,ref,re_step,re_level,content,ip,readcount,r  " +
            	    "from (select num,writer,email,subject,passwd,reg_date,ref,re_step,re_level,content,ip,readcount,rownum r " +
            	    "from (select num,writer,email,subject,passwd,reg_date,ref,re_step,re_level,content,ip,readcount " +
            	    "from reply_board where " + searchValue + " like '%" + searchText + "%' order by ref desc, re_step asc) order by ref desc, re_step asc ) where r >= ? and r <= ? "
            		);
            pstmt.setInt(1, start);
            pstmt.setInt(2, end);
            rs = pstmt.executeQuery();
